...
while running:
    ...
    #store sensor readings of all sensors as well as predicted pos pitch and yaw in data.csv
    if not args.offline:
        with open('data.csv', mode='a') as data_file:
            ...
            for i in range(TOTAL_SENSOR_COUNT):
                if i in working_indices:
                    idx = working_indices.index(i)
                    row.extend(measured_B_raw[idx])
                else:
                    row.extend([None, None, None])
            row.extend([stylus_pos[0], stylus_pos[1], stylus_pos[2], pitch, yaw])
